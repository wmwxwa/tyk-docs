...
`reponse`
Used by response plugins. The fields are populated with the upstream HTTP response data. All the field contents can be modified. See `ResponseObject` for more details.
...
`oauth_keys`
The keys are set by Tyk if the token is generated by an OAuth client during an OAuth authorisation flow.

`basic_auth_data`
This section defines the basic auth password and hashing method.

`jwt_data`
This section contains a JWT shared secret if the key of an inbound JWT matches the ID of a token issued by Tyk.
...
`monitor`
Contains the percentage usage thresholds for user and/or organisation quotas. When the usage thresholds are exceeded Tyk will trigger a notification via webhooks.
