...
It applies to endpoints like /health for health checks, /node for node information
...
Enable SSL/TLS connection between your Tyk MDCB & Redis.
...
Determines the driver used by the storage. It could be `mongo-go` to use the official mongo driver for go or `mgo` to use the old driver.
