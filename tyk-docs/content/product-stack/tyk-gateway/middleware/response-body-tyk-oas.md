...
In this example the response body transform middleware has been configured for HTTP `PUT` requests to the `/anything` endpoint. The `body` contains a base64 encoded Go template (which you can check by pasting the value into a service such as [base64decode.org](https://www.base64decode.org)).
...
httpbin.org will respond with the original payload in the response and, if you do not have the response body transform middleware enabled, Tyk will return:
``` .json
{
    "value1": "world",
    "value2": "hello"
...
If, however, you enable the response body transform middleware, Tyk will modify the response to have this content:
``` .json
{
    "req-header": "bar",
    "req-param": "[foo]",
...
Adding Respsonse Body Transformation to your API endpoints is easy when using the API Designer in the Tyk Dashboard, simply follow the steps taken in this short video:
